<div class="max-w-screen-xl px-4 pt-20 pb-2 mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 lg:pt-5 up"
    ...
    {{-- ROLETA PADRÃO --}}
    <div class="relative w-96 h-96 mx-auto rounded-full border-4 overflow-hidden border-gray-300">
        <div id="wheel" class="w-full h-full rounded-full transition-transform ease-linear"
             :style="`transform: rotate(${angle}deg); transition-duration: ${speed}s;`">

            {{-- BLOCO CONDICIONAL --}}
            @if($isGenerated && count($names) > 0)
            ...
            @else
                @foreach(['red', 'blue', 'green', 'yellow', 'grey', 'purple'] as $i => $color)
                    <div class="absolute top-0 w-full h-full origin-center"
