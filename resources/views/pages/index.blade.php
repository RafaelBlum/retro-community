            ...
            @if($posts->count() != 0)
                <div class="grid gap-8 sm:grid-cols-2 {{ $posts->count() < 3 ? 'lg:grid-cols-2 justify-items-center max-w-2xl mx-auto' : 'lg:grid-cols-3' }}">
                    @foreach($posts as $post)
                        <article
                            class="group bg-gray-50 dark:bg-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                            <a href="{{ route('posts.post', ['slug' => $post->slug]) }}" class="block overflow-hidden">
                                <img src="{{ Storage::url($post->featured_image_url) }}" alt=""
                                    class="w-full h-52 object-cover group-hover:scale-105 transition-transform duration-500">
                            </a>
                            <div class="p-6">
                                <a href="{{ route('posts.category', ['slug' => $post->category->slug]) }}"
                                    class="inline-block px-3 py-1 text-xs font-semibold uppercase rounded-full bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300 mb-3">
                                    {{ $post->category->title }}
                                </a>
                                ...
                                <div class="flex items-center gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
                                    <img src="{{ Storage::url($post->author->channel->brand) }}"
                                        class="w-9 h-9 rounded-full object-cover ring-2 ring-gray-200 dark:ring-slate-600">
                                    <div>
