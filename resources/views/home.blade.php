        ...
        <div class="relative max-w-screen-xl px-4 pt-32 pb-56 mx-auto lg:pt-44">
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="relative flex justify-center">
                    ...
                    <img src="{{ asset('images/hero-4.png') }}" alt="hero" class="vertical-loop-animation absolute inset-0 w-[400px] h-auto mx-auto z-20">
                </div>
            </div>
        </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                        @foreach($followedChannels as $channel)
                            <a href="{{ route('my.channel', $channel->slug) }}" class="group text-center">
                                <div class="w-24 h-24 mx-auto rounded-2xl overflow-hidden ring-2 ring-transparent group-hover:ring-purple-500 transition-all group-hover:scale-105 shadow-lg">
                                    <img src="{{ Storage::url($channel->brand) }}" class="w-full h-full object-cover">
                                </div>
                                <p class="mt-3 text-sm font-medium text-gray-700 dark:text-gray-300 truncate">{{ $channel->name }}</p>
                            </a>
                        @endforeach
                    </div>
                </div>
            ...
            @if($channels->isNotEmpty())
                <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach($channels as $channel)
                        <div class="group p-6 rounded-2xl bg-white dark:bg-slate-800 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                            <div class="w-28 h-28 mx-auto mb-4 rounded-full overflow-hidden ring-4 ring-gray-200 dark:ring-slate-700 group-hover:ring-purple-500 transition-all">
                                <img src="{{ Storage::url($channel->brand) }}" class="w-full h-full object-cover" alt="{{ $channel->name }}">
                            </div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                                <a href="{{ route('my.channel', $channel->slug) }}" class="hover:text-purple-600 transition-colors">
                                    {{ $channel->name }}
                                </a>
                            </h3>
                ...
                <div class="mt-12">
                    <a href="{{ route('app.channels') }}" class="inline-flex items-center gap-2 text-sm font-medium text-purple-600 hover:text-purple-700 transition-colors group">
                        Ver todos os canais
        ...
        <div class="max-w-screen-xl px-4 mx-auto">
            ...
            @if($posts->count() > 0)
                ...
                <div class="mt-12 text-center">
                    <a href="{{ route('posts.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-purple-600 hover:text-purple-700 transition-colors group">
